...
class Admin::OverviewController < Admin::BaseController
  ...
  def best_selling_variants
    ...
    li = LineItem.order_checkout_state_eq("complete").sum(:quantity, :group => :variant_id, :limit => 5)
  ...
  def top_grossing_variants
    quantity = LineItem.order_checkout_state_eq("complete").sum(:quantity, :group => :variant_id, :limit => 5)
    prices = LineItem.order_checkout_state_eq("complete").sum(:price, :group => :variant_id, :limit => 5)
    ...
    variants = quantity.map do |v|
      variant = Variant.find(v[0])
      [variant.name, variant.price*quantity[v[0]]]
    end
  ...
  def out_of_stock_products
    #Variant.find(:all, :conditions => {:count_on_hand => 0}, :limit => 5)
    #Variant.count_on_hand_eq(0).is_master_eq(false)
    Variant.count_on_hand_eq(0)
  end
end
